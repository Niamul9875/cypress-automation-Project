...
import TimeDepositCshTrans from '../../BankUltimus/Deposit/TimeDepositCashTransPage'; // ES Module import
...
describe('Bank Ultimus', () => {
    ...
    it('Step 4: Beneficiary Authorize & Transaction', function () {
        ...
        const menuSearch = new MenuSearch();
        const nftAuthorize = new NFTAuthorize();
        const tdCshTrans = new TimeDepositCshTrans();
        const logoutPage = new LogoutPage();
        //Login with valid Checker User ID
        cy.task('readExcel', {
            fileName: 'loginData3.xlsx',
        ...
        cy.task('readExcel', {
         ...
         //Go to TDCashTransaction Page (FP: 7003)
        cy.task('readExcel', {
            fileName: 'loginData3.xlsx',
            sheetName: 'MenuSearch'
        }).then((dataMenuSearch) => {
            Cypress.env('excelData', dataMenuSearch[19]); // Use first row
            menuSearch.menu();
        });
        ...
        cy.task('readExcel', {
            ...
            Cypress.env('excelData', dataTDcshTrans[0]); // Use first row
            tdCshTrans.TDCashTrans();
        });
