...
import CollateralsCollection from '../../BankUltimus/Common/CollateralsCollectionPage'; // ES Module import
...
describe('Bank Ultimus', () => {
    it('Step 1: Financial Proposal Register', function () {
        ...
        cy.task('readExcel', {
            ...
            Cypress.env('excelData', dataFinancialProposal[2]); // Use first row
    ...
    it('Step 3: Financing Approval Authorization & Commitment Portfolio Register', function () {
     ...
     it('Step 4: Commitment Authorization & Collaterals Collection', function () {
        const loginPage = new LoginPage();
        const menuSearch = new MenuSearch();
        const logoutPage = new LogoutPage();
        const nftAuthorize = new NFTAuthorize(); 
        const collateralsCollection = new CollateralsCollection();
        ...
        cy.task('readExcel', {
            ...
            Cypress.env('excelData', dataMenuSearch[27]); // Use first row
            menuSearch.menu();
        });
        // // Fill up all information at 3151
        cy.task('readExcel', {
            fileName: 'loginData3.xlsx',
            sheetName: 'CollateralsCollection'
        }).then((dataCollateralsCollection) => {
            Cypress.env('excelData', dataCollateralsCollection[0]); // Use first row
            collateralsCollection.CollateralsCollection();
        });

        logoutPage.Logout();//logout maker user   

    });
    it('Step 5: Collaterals Collection Authorization & Loan Account Open', function () {
        const loginPage = new LoginPage();
        const menuSearch = new MenuSearch();
        const logoutPage = new LogoutPage();
        const nftAuthorize = new NFTAuthorize();
        const lon_AccountOpen = new Lon_AccountOpen();
        ...
        cy.task('readExcel', {
            ...
            Cypress.env('excelData', dataNftAuth[12]); // Use first row
            nftAuthorize.NftAuth();
        });
        //Go to Financing Approval Page (FP: 3131)
        cy.task('readExcel', {
            ...
            Cypress.env('excelData', dataMenuSearch[21]); // Use 2nd row
            menuSearch.menu();
        });
       // Fill up all information at 3121
        cy.task('readExcel', {
            fileName: 'loginData3.xlsx',
            sheetName: 'LoanAccount'
        }).then((dataLoanAccount) => {
            Cypress.env('excelData', dataLoanAccount[0]); // Use first row
            lon_AccountOpen.LoanAccountOpenPage();
        });

        logoutPage.Logout();//logout maker user   

    });
    it('Step 6: Loan Account Authorization & Loan Account Disbursement', function () {
        const loginPage = new LoginPage();
        const menuSearch = new MenuSearch();
        const nftAuthorize = new NFTAuthorize();
        const logoutPage = new LogoutPage();
        const lonAllowLimitSetup = new LonAllowLimitSetup();
        ...
        cy.task('readExcel', {
            ...
            Cypress.env('excelData', dataNftAuth[9]); // Use first row
            nftAuthorize.NftAuth();
        });
         //Go to Financing Approval Page (FP: 3131)
        cy.task('readExcel', {
    ...
    it('Step 8: Transaction Authorization & Loan Account Repayment', function () {
        const loginPage = new LoginPage();
        const menuSearch = new MenuSearch();
    ...
    it('Step 9: Transaction Authorization & Balance Inquary', function () {
        const loginPage = new LoginPage();
        const menuSearch = new MenuSearch();
        const transAuth = new TransAuth();
