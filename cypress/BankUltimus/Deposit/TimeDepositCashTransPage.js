class TimeDepositCashTrans {

    TDCashTrans() {


        const dataTDCashTrans = Cypress.env('excelData');
        ...
        //cy.get('#ctl00_contPlcHdrMasterHolder_LstxtAccNo').focus().clear().type(dataTDCashTrans.Acc_no).type('{enter}');
        //cy.wait('@formReload');
        ...
        // cy.get('#ctl00_contPlcHdrMasterHolder_LstxTransAmtCcy').clear();
         cy.wait(3000);
        cy.get('#ctl00_contPlcHdrMasterHolder_LstxTransAmtCcy').clear();
        cy.get('#ctl00_contPlcHdrMasterHolder_LstxTransAmtCcy').type(dataTDCashTrans.amount).type('{enter}');
        ...
        cy.get('#ctl00_contPlcHdrMasterHolder_LstxtNarration').type(dataTDCashTrans.narration).type('{enter}');
        cy.wait(3000);
        cy.get('#ctl00_contPlcHdrMasterHolder_lsddlDepositorBeareType').focus().blur();
        //cy.wait('@formReload');       //intercept Post Request call
        cy.get('#ctl00_contPlcHdrMasterHolder_lsddlDepositorBeareType').focus().select(dataTDCashTrans.beare_type, { force: true });
        ...
        cy.get('#dialog') // or the actual selector that targets the text container
            ...
            .then((popupText) => {
                ...
                Cypress.env('transMode','Cash');
               cy.log(`trans Mode: ${Cypress.env('transMode')}`);
...
export default TimeDepositCashTrans;